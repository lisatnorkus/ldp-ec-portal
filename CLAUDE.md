...
2. **Passphrase auth ships first; Google OAuth deferred to post-testing.** v2.0 launches with the same passphrase gate as v1 (client-side soft gate, sessionStorage-backed). A handful of trusted users test the full portal before OAuth is added. Only after that test loop does OAuth land — at which point passphrase is retired. Do NOT wire Google OAuth in the initial v2.0 build. The auth gate component should be scaffolded so OAuth slots in without rewriting, but the actual Google sign-in flow is deliberately deferred.
...
- **Auth (initial v2.0)**: **Passphrase soft-gate only** — same mechanism as v1 (plain-string check, sessionStorage-backed). Shipped so a handful of trusted users can walk the portal before OAuth is added. The auth gate component is scaffolded with an `AuthProvider` wrapper so the Google OAuth flow slots in later without touching page code.
- **Auth (post-test loop)**: Google OAuth via Supabase Auth (native integration). Users sign in with their **personal Gmail** — the same account LDP invited them with. Restrict access by matching the signed-in email against `ec_members.google_oauth_email`. No match → render a "request access" screen pointing at `communications@louisvilledems.com`. Match → route to dashboard. Passphrase retired once OAuth is live.
- **Explicitly deferred (post-v2.0 or post-test)**: Google OAuth (see above) and Google Workspace / `@louisvilledems.com` domain integration. Do NOT configure Workspace-level OAuth restrictions, domain-wide policies, or `hd=` parameter enforcement in v2.0. When OAuth lands, it will allowlist by `ec_members.google_oauth_email` column only.
...
- `highest-leverage-rules.md` — v1 rules table for the Step 3 "Your highest-leverage move this week" recommendation engine. **Lisa's red-pen applied April 21, 2026.** 14 active rules (Rule 8 deleted, Rule 15 deferred to post-June chair transition). Tone softened throughout — recommendations are supportive opportunities, not obligations. Engine is buildable; data flow for the `raise_progress_dollars` variable (ticket-link sales tracking) is stubbed until that integration lands.
- **$120 Club math (correction via red-pen):** $120 personal give ($10/mo auto-pay) + $500 raised = member commitment. The $500 raise is driven primarily by personalized ticket-sale links that go out 30 days before each signature event (COD Dinner / WDD / Dems at the Downs). Every ticket sold through a member's link counts toward their $500. The dashboard's "$120 Club" card currently shows members × $620 floor; that's the aggregate structural floor, not per-person — update copy accordingly if confusion arises.
- **Kentucky early-voting dates:** KY runs 3-day no-excuse early voting Thu/Fri/Sat before Election Day. The rules engine treats the Thu→Tue window as LOCKDOWN (primary or general), not just Election Day itself. For 2026: PRIMARY_LOCKDOWN = May 14–19, GENERAL_LOCKDOWN = Oct 29–Nov 3.
